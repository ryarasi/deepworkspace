...
This command reads CLAUDE.md from the current directory (or parent directories)
...
class ProjectContext:
    """Extracted project context from CLAUDE.md."""
    path: Path
    purpose: str
    rules: List[str]
    workflows: List[str]
...
class ContextParser:
    """Parses CLAUDE.md files to extract project context."""
...
class AlignmentChecker:
    ...
    def _check_directory_alignment(self):
        ...
        # If CLAUDE.md is in a parent directory, might be okay
...
class SyncCommand:
    ...
    def find_claude_md(self) -> Optional[Path]:
        """Find CLAUDE.md file in current or parent directories."""
        current = Path.cwd()
    ...
    def load_context(self, path: Path) -> ProjectContext:
        """Load and parse project context from CLAUDE.md."""
        try:
