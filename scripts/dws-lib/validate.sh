...
check_structural_logic() {
    ...
    for rule_file in "$WORKSPACE_ROOT/rules"/*.yaml; do
        ...
        while IFS= read -r line; do
            ...
            if [[ "$in_refs" == true ]] && [[ "$line" =~ ^[[:space:]]*-[[:space:]]*\"([RT][0-9]+)\" ]]; then
                ...
                if [[ "$ref" =~ ^R ]]; then
                    if ! ls "$WORKSPACE_ROOT/rules/${ref}"*.yaml >/dev/null 2>&1; then
                        issues+=("$(basename "$rule_file") references missing rule: $ref")
                    fi
                elif [[ "$ref" =~ ^T ]]; then
                    if ! ls "$WORKSPACE_ROOT/templates/${ref}"*.yaml >/dev/null 2>&1; then
    ...
    for i in {001..010}; do
        if ! ls "$WORKSPACE_ROOT/rules/R${i}"*.yaml >/dev/null 2>&1; then
