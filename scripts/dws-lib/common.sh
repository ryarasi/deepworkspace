...
# Get workspace root (traverse up until we find README.md with DeepWorkspace metadata)
get_workspace_root() {
    local current_dir="$(pwd)"
    while [[ "$current_dir" != "/" ]]; do
        if [[ -f "$current_dir/README.md" ]] && grep -q "^# DeepWorkspace" "$current_dir/README.md" 2>/dev/null; then
            echo "$current_dir"
            return 0
        fi
