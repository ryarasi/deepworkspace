...
if [[ ! -f "$WORKSPACE_ROOT/README.md" ]] || [[ ! -d "$WORKSPACE_ROOT/scripts" ]]; then
...
if [[ -n "$ZSH_VERSION" ]]; then
    ...
    case "$SHELL" in
        ...
        *)
            ...
            echo
            echo "export PATH=\"\$PATH:$WORKSPACE_ROOT/scripts\""
            exit 1
            ;;
    esac
fi
...
if grep -q "deepworkspace/scripts" "$SHELL_CONFIG" 2>/dev/null; then
...
export PATH="\$PATH:$WORKSPACE_ROOT/scripts"
