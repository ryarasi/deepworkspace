#!/bin/bash
# check-branch.sh - Verify git branch compliance for DeepWork
...
show_branch_status() {
    ...
    if [ "$CURRENT_BRANCH" = "main" ] || [ "$CURRENT_BRANCH" = "master" ]; then
        ...
        echo -e "  ${GREEN}./scripts/safe-edit.sh${NC}"
...
suggest_next_steps() {
    ...
    if [ "$CURRENT_BRANCH" = "main" ] || [ "$CURRENT_BRANCH" = "master" ]; then
        ...
        echo -e "3. Create PR with: ${GREEN}./scripts/create-pr.sh${NC}"
    fi
}

# Main execution
echo -e "${BLUE}DeepWork Branch Compliance Check${NC}"
echo -e "${BLUE}=====================================/${NC}"
