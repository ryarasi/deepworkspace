#!/bin/bash
# Common functions for Project CLI

# Get project root (traverse up until we find docs/rules/ directory)
get_project_root() {
    local current_dir="$(pwd)"
    while [[ "$current_dir" != "/" ]]; do
        if [[ -d "$current_dir/docs/rules" ]]; then
            echo "$current_dir"
            return 0
        fi
        current_dir="$(dirname "$current_dir")"
    done
    echo "Error: Not in a project directory with docs/rules/" >&2
    return 1
}
